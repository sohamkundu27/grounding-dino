#!/usr/bin/env python3
"""Fully-offline open-vocabulary video annotation: Grounding DINO + SAM 2 (downsampled).

This is `annotate_video.py` with DOWNSAMPLING added so it runs faster. Everything else --
models, prompts, thresholds, offline enforcement, tracking behaviour, rendering, CLI
surface -- is unchanged. See that file for the full behavioural reference notes; only the
downsampling differences are documented here.

Grounding DINO turns a natural-language prompt into boxes on the first frame; those boxes
seed the SAM 2 video predictor, which propagates masks across every remaining frame.

DOWNSAMPLING (the only difference from annotate_video.py)
--------------------------------------------------------
Two independent knobs, BOTH ON BY DEFAULT. Set --frame-stride 1 --max-side 0 to reproduce
annotate_video.py's output exactly.

  --frame-stride N (default 2)  Temporal. Only every Nth source frame is decoded, fed to
      the models, and written. The output has ceil(source_frames / N) frames at
      source_fps / N, so the clip's wall-clock DURATION is preserved while the pipeline
      does N times less work. Skipped frames are dropped, not re-drawn with stale masks:
      every frame in the output carries masks computed for that frame.

  --max-side P (default 1280)   Spatial. Frames whose longer edge exceeds P pixels are
      resized (INTER_AREA) so the longer edge is P, preserving aspect ratio, before
      anything else sees them. The output MP4 is written at that reduced resolution.
      Videos already within P are passed through untouched. 1280 turns 1080p into 720p.
      0 disables the cap.

What this buys, and what it costs:
  * Faster: N times fewer SAM 2 propagation steps and JPEG writes; skipped frames are
    grabbed but never decoded (cv2.grab/retrieve); decode, resize, mask compositing and
    text rendering all run at the reduced resolution; SAM 2's init_state loads a smaller,
    shorter frame set, so peak memory drops too.
  * SAM 2's per-frame compute is set by its config's image_size, so --max-side mainly
    speeds up I/O, loading and rendering rather than the transformer itself.
    --frame-stride is the knob that cuts model work.
  * Detection quality: Grounding DINO runs on the DOWNSCALED frame 0. Small or distant
    objects that were detectable at full resolution may fall below --box-threshold. If a
    prompt stops matching, raise --max-side before lowering the threshold.
  * Tracking quality: a larger stride means more motion between consecutive frames, which
    is what SAM 2's memory has to bridge. Fast motion tracks less reliably at stride 4+.
  * --redetect-interval is counted in PROCESSED frames, not source frames: with
    --frame-stride 2 --redetect-interval 30, re-detection happens every 60 source frames.
...
3. Downsampling off (byte-for-byte equivalent to annotate_video.py):
     ...
     python annotate_video_fast.py --frame-stride 1 --max-side 0
...
4. Expected output behaviour:
     ...
     - Output frame count, FPS and resolution match the DOWNSAMPLED targets, and the
       finished file is reopened and verified against them before success is reported.
     - If the prompt matches nothing, a valid MP4 of the UNMODIFIED (but downsampled)
       frames is still written and a warning is printed (exit code stays 0).
"""
...
# annotate_video.py. --frame-stride 1 --max-side 0 restores the original behaviour.
...
class VideoMeta:
    """Metadata of a video. `frame_count` is None when unreported.

    Used for both the source and, via `derive_output_meta`, the downsampled output.
    """
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "and tracks them through the rest of the video. Frames are downsampled in time "
            "(--frame-stride) and space (--max-side) so the run is faster."
        ),
    ...
    parser.add_argument(
        "--frame-stride", type=int, default=DEFAULT_FRAME_STRIDE, metavar="N",
        help="Temporal downsampling: process and write only every Nth source frame. The "
        "output has ceil(frames/N) frames at fps/N, so duration is preserved. 1 = every "
        "frame (no temporal downsampling).",
    )
    parser.add_argument(
        "--max-side", type=int, default=DEFAULT_MAX_SIDE, metavar="P",
        help="Spatial downsampling: cap the longer edge of every frame at P pixels "
        "(aspect ratio preserved, INTER_AREA). The output is written at that resolution. "
        "Videos already within P are untouched. 0 = no cap.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "self-cleaning temporary directory. The JPEGs are the downsampled frames.",
    )
    parser.add_argument(
        "--redetect-interval", type=int, default=0, metavar="N",
        help="Re-run Grounding DINO every N PROCESSED frames to catch objects that appear "
        "later (0 = detect once on the first frame). With --frame-stride S this is every "
        "N*S source frames. See TRACKING LIMITATIONS in annotate_video.py's docstring: "
        "N>0 resets SAM 2 state every N frames and rematches IDs by box IoU.",
    )
    parser.add_argument(
    ...
    if args.frame_stride < 1:
        parser.error("--frame-stride must be >= 1 (1 = keep every frame)")
    if args.max_side != 0 and args.max_side < 64:
        parser.error("--max-side must be 0 (no cap) or >= 64")
...
def scaled_size(width: int, height: int, max_side: int) -> tuple[int, int]:
    """Size with the longer edge capped at `max_side`, aspect ratio preserved.

    Returns the input size unchanged when `max_side` is 0 or already large enough, so a
    small source video is never upscaled and never re-encoded through a resize. Both
    dimensions are rounded to even numbers, which several MP4 encoders require.
    """
...
def derive_output_meta(meta: VideoMeta, stride: int, max_side: int) -> VideoMeta:
    """The geometry/FPS the output is written at, after temporal + spatial downsampling.

    FPS is divided by the stride so playback duration matches the source: dropping every
    other frame AND keeping the source FPS would play the clip back at 2x speed.
    `frame_count` is the expected processed count derived from the source metadata; the
    authoritative count is what extraction actually decodes.
    """
    width, height = scaled_size(meta.width, meta.height, max_side)
    frame_count = math.ceil(meta.frame_count / stride) if meta.frame_count else None
    return replace(meta, width=width, height=height, fps=meta.fps / stride, frame_count=frame_count)
...
def open_video_writer(output_path: Path, meta: VideoMeta, codec: str) -> cv2.VideoWriter:
    """Open a VideoWriter at the given meta's resolution and FPS, or fail loudly.

    `meta` here is the DOWNSAMPLED output meta, not the source's.
    """
...
def extract_frames_to_dir(
    video_path: Path, frames_dir: Path, jpeg_quality: int, stride: int, size: tuple[int, int],
) -> int:
    """Decode every `stride`-th frame at `size` into zero-padded JPEGs, which SAM 2 ingests.
    ...
    int(os.path.splitext(p)[0]) -- and the index is the PROCESSED index (source index //
    stride), so the JPEG set is a contiguous 0..M-1 range regardless of stride.

    Skipped frames are grabbed but never decoded (`grab` without `retrieve`), which is
    where a large part of the temporal speedup comes from. Returns the number written.
    """
    cap = cv2.VideoCapture(str(video_path))
    try:
        if not cap.isOpened():
        ...
        count = 0        # frames actually written (the processed index)
        source_idx = 0   # position in the source video
        while True:
            if not cap.grab():
                break
            if source_idx % stride == 0:
                ...
                if not ok or frame is None:
                    break
                if frame.shape[1::-1] != size:  # (w, h) from (h, w, c)
...
def validate_output_video(
    ...
    `meta` is the DOWNSAMPLED output meta, so this verifies the file against the
    downsampled targets -- the geometry and FPS we actually intended to write, not the
    source's. Raises RuntimeError on any failure so the run cannot report success on a
    video that does not open, lost frames, changed geometry, or has no annotations at all.
    """
    ...
    if fps <= 0 or abs(fps - meta.fps) > 0.01:
        raise RuntimeError(f"Output FPS {fps:.3f} does not match the target {meta.fps:.3f}.")
...
class SequentialFrameReader:
    """Pull original (non-recompressed) frames by PROCESSED index, seeking only when needed.

    Processed index `i` maps to source frame `i * stride`; intervening frames are grabbed
    without being decoded. Frames are resized to `size` on the way out, so what is
    rendered and written matches the downsampled JPEGs SAM 2 produced masks from. Reading
    from the source rather than the extracted JPEGs keeps the output free of a second
    round of JPEG compression artefacts.
    """

    def __init__(self, video_path: Path, stride: int = 1, size: tuple[int, int] | None = None) -> None:
        self._cap = cv2.VideoCapture(str(video_path))
        if not self._cap.isOpened():
            raise RuntimeError(f"Could not re-open video for rendering: {video_path}")
        self._stride = stride
        self._size = size
        self._next_idx = 0  # position in the SOURCE video

    def read(self, index: int) -> np.ndarray | None:
        """Return processed frame `index` in BGR at the output size, or None past the end."""
        source_idx = index * self._stride
        if source_idx < self._next_idx:  # rewind (propagation normally never goes backwards)
            self._cap.set(cv2.CAP_PROP_POS_FRAMES, source_idx)
            self._next_idx = source_idx
        while self._next_idx < source_idx:  # skip forward without decoding
...
def detect_objects(
    ...
    scale by (w, h, w, h) and convert -- the conversion the upstream demos perform. The
    frame handed in is already downsampled, so the boxes are in downsampled pixels, which
    is the coordinate system SAM 2 and the renderer both work in.
    """
...
def draw_annotations(frame: np.ndarray, tracks: Sequence[Track]) -> np.ndarray:
    ...
    Font scale and line thickness key off the frame height, so a downsampled frame gets
    proportionally smaller annotations rather than text that swamps the image.
    """
...
def _tracks_from_logits(obj_ids: Sequence[int], mask_logits, id_to_label, id_to_conf, shape: tuple[int, int]) -> list[Track]:
    ...
    `shape` is the DOWNSAMPLED (height, width): SAM 2 read the downsampled JPEGs, so its
    masks come back at that size and line up with the frames the renderer draws on.
    """
    height, width = shape
    tracks: list[Track] = []
...
class _FrameEmitter:
    ...
    All indices here are PROCESSED indices (0..total_frames-1 over the downsampled
    frames), which is also how SAM 2 numbers the JPEGs it was given.
    """
    ...
    def drain(self) -> None:
        """Copy any remaining processed frames through unmodified (keeps the count exact)."""
...
def _propagate_with_redetection(
    ...
    """Re-detect every N processed frames, following the upstream continuous-id demo.
    ...
    preserved by matching fresh detections to each track's last known box with IoU. The
    interval counts DOWNSAMPLED frames, so with --frame-stride S a chunk spans N*S source
    frames.
    """
...
def write_annotated_video(
    ...
    seeded frame. `meta` and `total_frames` are the downsampled output's. Returns
    (frames_written, frames_containing_at_least_one_object).
    """
    ...
    emitter.drain()  # keep the output frame count identical to the processed count
...
def write_passthrough_video(reader: SequentialFrameReader, writer: cv2.VideoWriter, total_frames: int) -> int:
    """Copy every processed frame through unmodified (used when nothing is detected)."""
    emitter = _FrameEmitter(reader, writer, total_frames)
    emitter.drain()
...
def _run(args: argparse.Namespace) -> int:
    ...
    # State the downsampling explicitly: every later number (frame counts, FPS, geometry)
    # refers to the downsampled output, and a silent mismatch would be confusing.
    if args.frame_stride > 1 or out_size != (meta.width, meta.height):
        print(f"[downsample] stride={args.frame_stride} (fps {meta.fps:.3f} -> {out_meta.fps:.3f})  "
              f"max_side={args.max_side or 'off'} ({meta.width}x{meta.height} -> "
              f"{out_meta.width}x{out_meta.height})")
    else:
        print("[downsample] off (--frame-stride 1, no resize): identical to annotate_video.py")
    ...
    with frames_directory(args.keep_frames_dir) as frames_dir:
        ...
        if meta.frame_count is None:
            print(f"[video] Frame count metadata was missing; extracted {total_frames} frames "
                  f"at stride {args.frame_stride}.")
        elif out_meta.frame_count != total_frames:
            print(f"[warn] Metadata implied {out_meta.frame_count} processed frames; extracted "
                  f"{total_frames}. Using the extracted count.")
        print(f"[video] processing {total_frames} frame(s) at {out_meta.width}x{out_meta.height} "
            ...
            with torch.inference_mode():
                ...
                if not detections:
                    print(f"[warn] No objects matched {args.text_prompt!r} on the first frame. "
                          "Writing the original frames unmodified; try raising --max-side "
                          "(detection ran on the downsampled frame) or lowering --box-threshold.")
                    ...
                    written, frames_with_objects = write_annotated_video(
                        predictor, state, reader, writer, out_meta, total_frames,
