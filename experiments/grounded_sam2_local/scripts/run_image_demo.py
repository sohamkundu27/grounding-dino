#!/usr/bin/env python3
"""PHASE 1 — smoke test: one image through the fully-local pipeline.

  text prompt -> Grounding DINO 1.0 (local) -> box -> SAM 2.1 (local) -> mask
...
def visualise(img_rgb, boxes, scores, phrases, masks, prompt, path: Path):
    ...
    axes[2].set_title("SAM 2.1 masks", fontsize=13)
...
def main() -> int:
    ...
    load_ms = t[0]
    print(f"model load: {load_ms:.0f} ms  (GD Swin-T + SAM2.1 {args.sam2})")
    ...
    for prompt in args.prompts:
        ...
        if n:
            masks, sam_ms = G.sam2_masks(sam, img_rgb, boxes)
        ...
        print(f"  '{prompt}': {n} det  gd={gd_ms:.0f}ms"
              + (f"  sam2={sam_ms:.0f}ms" if sam_ms else "  (no box -> SAM2 skipped)")
