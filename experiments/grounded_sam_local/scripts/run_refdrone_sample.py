#!/usr/bin/env python3
"""PHASE 2 — deterministic RefDrone sample through the local Grounded SAM (v1) pipeline.

For each sampled (image, referring-expression) pair:
  expression -> Grounding DINO -> highest-confidence box -> SAM v1 -> mask

Metrics are computed against RefDrone's GROUND-TRUTH BOXES only. RefDrone provides
no mask ground truth, so SAM v1 mask quality is NOT scored here -- it is assessed
visually. No segmentation ground truth is invented.

The detector, sampler (same seed), and thresholds are identical to the SAM 2
evaluation, so the boxes scored here are the SAME boxes SAM 2.1 was scored on. The
localization numbers are therefore expected to match the SAM 2 run exactly; the only
thing that changes downstream of the box is which network draws the mask. That is the
point -- it isolates the SAM stage.
...
def main() -> int:
    ...
    for i, r in enumerate(sample):
        ...
        if n_det:
            ...
            masks, sam_ms = G.sam_masks(sam, img_rgb, np.array([best_box]))
    ...
    summary = {
        ...
        "detector_note": ("Grounding DINO stage is identical to the SAM 2 evaluation "
                          "(same build, config, checkpoint, thresholds and seed). The "
                          "localization metrics below are a property of the detector and "
                          "are expected to equal the SAM 2 run's. Only mask_pixels and "
                          "sam_latency_ms belong to the SAM v1 stage."),
        "metrics": {
            ...
            "mean_sam_latency_ms": round(
                float(np.mean([r["sam_latency_ms"] for r in det])), 1) if det else None,
            "mean_total_latency_ms": round(float(np.mean(
                [r["gd_latency_ms"] + (r["sam_latency_ms"] or 0) for r in rows])), 1),
        },
        "gpu_memory": G.gpu_mem_mb(),
        "mask_ground_truth": "NONE — RefDrone provides boxes only. SAM v1 mask quality "
    ...
    print(f"  mean GD / SAM v1 latency     : {m['mean_gd_latency_ms']:.0f} / "
          f"{m['mean_sam_latency_ms']:.0f} ms")
